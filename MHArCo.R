...
######MH
#remove unnecessary data in environment
rm(compliance_data)
rm(compliance_data)
rm(installation_data)
rm(joined_df)
rm(df)
rm(df2)
rm(col_order)
...
alpha = 0.05
boot.cf = FALSE
R = 100 #only in the case if boot.cf = TRUE
l = 1
# VCOV.type=c("iid","var","nw","varhac")
VCOV.type="nw"
VCOV.lag=1
bandwidth.kernel=NULL
# kernel.type=c("QuadraticSpectral","Truncated","Bartlett","Parzen","TukeyHanning")
kernel.type = "TukeyHanning"
VHAC.max.lag=5
prewhitening.kernel=FALSE


MHArCo=function (data, fn=NULL, p.fn=NULL, treated.unit, t0, lag = 0, Xreg = NULL, alpha = 0.05, boot.cf = FALSE, R = 100, l = 3,VCOV.type=c("iid","var","nw","varhac"),VCOV.lag=1,bandwidth.kernel=NULL,kernel.type=c("QuadraticSpectral","Truncated","Bartlett","Parzen","TukeyHanning"),VHAC.max.lag=5,prewhitening.kernel=FALSE,...) 
{
