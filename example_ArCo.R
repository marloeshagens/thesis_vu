...
#############################
data(data.q1)
data.q1 <- data.q1[-c(16:100),]
# = First unit was treated on t=51 by adding
# a constant equal to one standard deviation
example=list(data.q1) # = Even if q=1 the data must be in a list
...
ArCo=fitArCo(data = example,fn = fn, p.fn = p.fn, treated.unit = 1 , t0 = 5)

###################################################
## === PLOT Example based on the q=1 fitArCo === ##
###################################################
# = First unit was treated on t=51 by adding
# a constant equal to one standard deviation
data(data.q1)
...
plot(ArCo)
plot.fitArCo(ArCo) #same as previous
...
ArCo2=fitArCo(data = data.q2,fn = cv.glmnet, p.fn = predict,treated.unit = 1 , 
      t0 = 51,VCOV.type = "nw",kernel.type = "QuadraticSpectral",VCOV.lag = 2)
